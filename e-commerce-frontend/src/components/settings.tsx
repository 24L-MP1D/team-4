            ...
            <div className="flex flex-col gap-2">
                {settings.map((setting) =>
                    <div className="flex justify-between items-center border border-tagGrayBg rounded-xl py-[14px] px-3">
                        ...
                        <button className={`text-buttonPrimary font-semibold text-sm rounded-[8px] py-2 px-4 border border-tagLightGrayBg  }`}  onClick={()=> setOpen(true) }>{setting.buttonname}</button> 
                    </div>
                )}
                <ShopType open={open} setOpen={setOpen}/>
