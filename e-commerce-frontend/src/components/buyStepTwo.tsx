                ...
                <div className="flex gap-5">
                    ...
                    <div className="w-[687px] bg-slate-400 rounded-2xl">
                        <div className="flex flex-col gap-6 p-8">
                            <div className="text-lg font-semibold">2. Хүргэлтийн мэдээлэл оруулах</div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-sm font-medium">Овог:</Label>
                                <Input className="rounded-full border-2"
                                    placeholder="Овог" id="firstName"
                                    value={formik.values.firstName} onChange={formik.handleChange}
                                />{formik.errors.firstName ? <span className="text-red-500 text-sm text-start">Овог оруулна уу</span> : null}
                            </div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-sm font-medium">Нэр:</Label>
                                <Input className="rounded-full border-2" placeholder="Нэр" id="lastName"
                                    value={formik.values.lastName} onChange={formik.handleChange}
                                />{formik.errors.lastName ? <span className="text-red-500 text-sm text-start">Нэр оруулна уу</span> : null}
                            </div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-sm font-medium">Утасны дугаар:</Label>
                                <Input className="rounded-full border-2" id="phoneNumber"
                                    value={formik.values.phoneNumber} onChange={formik.handleChange}
                                />{formik.errors.phoneNumber ? <span className="text-red-500 text-sm text-start">Утас аа заавал бичнэ үү</span> : null}
                            </div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-sm font-medium">Хаяг:</Label>
                                <Textarea className="rounded-[18px] h-24 border-2" id="homeAddress"
                                    value={formik.values.homeAddress} onChange={formik.handleChange}
                                />{formik.errors.homeAddress ? <span className="text-red-500 text-sm text-start">Хаяг тодорхой оруулна уу</span> : null}
                            </div>
                            <div className="flex flex-col gap-2">
                                <Label className="text-sm font-medium">Нэмэлт мэдээлэл:</Label>
                                <Textarea className="rounded-[18px] border-2 h-16" id="nemeltMedeelel"
                                    value={formik.values.nemeltMedeelel} onChange={formik.handleChange}
                                />{formik.errors.nemeltMedeelel ? <span className="text-red-500 text-sm text-start">Нэмэлт мэдээлэл заавал оруулна уу</span> : null}
                                <span className="text-xs font-normal">Хүргэлттэй холбоотой нэмэлт мэдээлэл үлдээгээрэй</span>
                            </div>
                            <div className="flex justify-between">
                                {/* <Button variant={"outline"} className="rounded-2xl px-9 py-2" onClick={() => formik.handleReset}>Буцах</Button> */}
                                <Button className="bg-primaryBlue hover:bg-blue-900 rounded-2xl px-9 py-2" type="submit" onClick={()=> formik.handleSubmit}>Төлбөр төлөх</Button>
                            </div>
                        </div>
                    </div>
                </div>
